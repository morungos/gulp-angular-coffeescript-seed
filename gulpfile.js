...
gulp.task('data-service', function () {
  return gulp.src([
    './src/service/data/**/*.*'
  ])
    .pipe(gulp.dest('./.tmp/src/service/data'));
});

gulp.task('test-service', ['coffee-service', 'data-service'], function () {
  return gulp.src([
    './.tmp/src/service/**/*_test.js',
  ], {read: false})
    .pipe(g.mocha({reporter: 'spec'}));
});

gulp.task('service-dist', ['coffee-service', 'data-service'], function() {
...
gulp.task('vendors', function () {
  var bowerStream = gulp.src(mainBowerFiles()).pipe(gulpIgnore.exclude('bower_components/bootstrap/**/*.*'));
  return es.merge(
...
gulp.task('build-all', ['styles', 'bootstrap', 'templates', 'coffee', 'coffee-service', 'data-service'], index);
function index () {
  var opt = {read: false};
  ...
  return gulp.src('./src/app/index.html')
    .pipe(g.inject(gulp.src(mainBowerFiles(opt)).pipe(gulpIgnore.exclude('bower_components/bootstrap/**/*.*')), {ignorePath: 'bower_components', starttag: '<!-- inject:vendor:{{ext}} -->'}))
...
gulp.task('dist', ['vendors', 'assets', 'styles-dist', 'scripts-dist', 'service-dist'], function () {
  return gulp.src('./src/app/index.html')
    .pipe(g.inject(gulp.src('./dist/statics/{bootstrap,vendors}.min.{js,css}'), {ignorePath: 'dist', starttag: '<!-- inject:vendor:{{ext}} -->'}))
